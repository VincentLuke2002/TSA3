<?php
// ACTIVITY A - BookTickets.php
// SELECT/INSERT/UPDATE, read from and write to the hardcoded infos
// catalog + session bookings list defined in Data.php.
...
$concert = getConcertById($concert_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$cat_id || $qty < 1 || $qty > 10) {
        ...
        $cat = getCategory($concert_id, $cat_id);
        ...
        if (!$cat) {
            ...
            // No INSERT here — the booking is appended to $_SESSION['bookings'].
            addBooking([
            ...
            // No UPDATE here — available_quantity is reduced directly
            // on the session copy of the catalog.
            reduceAvailability($concert_id, $cat_id, $qty);
        ...
        // Refresh categories so the form shows updated availability.
        $concert = getConcertById($concert_id);
        $categories = $concert['categories'];
    }
}
